...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        if compute_attn_matrix:
            ...
            # print("attention metrix for compute_attn_matrix_fn:", xai_matrix.shape)
            # print("SHAPES of metrices A, B, C HEERE:", A.shape, B.shape, C.shape)
...
def compute_middle_attn_vector(delta, delta_bias, A, B, C, L, x_shape, cls_first="middle", full_vector=False):
    # print("size of matrices A, B, C:", A.shape, B.shape, C.shape)
    # print("position of CLS token:", cls_first)
    ...
    ### instead of above line Azam added these lines:
    if C.shape[-1] == 37:
        cls_pos = 18
    elif C.shape[-1] == 6:
        # print("cls_pos in Encoder is the first token")
        cls_pos = 0
    else:
        raise ValueError(f"Unexpected last dimension in C: {C.shape[-1]}. Expected 37 or 6.")

    
    for t in range(cls_pos):
        # print("t:", t)
        curr_C = C[:,:,:,cls_pos]
        # print("curr_C shape:", curr_C.shape)
        currA = torch.ones(dA.shape[0],dA.shape[2],dA.shape[3]).to(dA.device)
        if t < (cls_pos-1):
            # print("AZZZZZAM ::::: t < cls_pos-1")
            for i in range(cls_pos-1-t):
                # print("i here is:", i)
                currA = currA*dA[:,cls_pos-1-i,:,:]
        currB = dB[:,t,:,:]
        AttnVecorOverCLS[:,:,t] = torch.sum(curr_C*currA*currB, axis=-1) 
    # print("AttnVecorOverCLS shape:", AttnVecorOverCLS.shape)
