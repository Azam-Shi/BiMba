...
import pymesh
import numpy as np
from masif.source.geometry.compute_polar_coordinates import compute_polar_coordinates
from sklearn import metrics
from scipy.spatial import cKDTree

def read_data_from_surface(ply_fn, config):
    """
    ...
    rho, theta, neigh_indices, mask = compute_polar_coordinates(mesh, radius=radius,
                                                                max_vertices=config['ppi_const']['max_shape_size'])
    ...
    n = len(mesh.vertices)
    input_feat = np.zeros((n, config['ppi_const']['max_shape_size'], 5))
    for vix in range(n):
