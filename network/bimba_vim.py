...
class VisionMambaWithFeatures(nn.Module):
    ...
    # def forward(self, x, extra_feat, return_features=False, inference_params=None):
    ...
    def forward(self, x, extra_feat, return_features=False, inference_params=None):
    # CLS embedding from ViM
        cls_embed = self.forward_features(x, inference_params)          # [B, D]

        # residue-level embedding
        extra_feat = self.extra_feats_encoder(extra_feat)                   # [B, 128]

        # combine CLS + residue features
        combined = torch.cat([cls_embed, extra_feat], dim=1)             # [B, D+128]
        combined = self.layernorm(combined)

        # final logit
        logits = self.cls_fc(combined).squeeze(-1)                      # [B]

        if return_features:
            # Return ALL embeddings for analysis
            return logits, cls_embed, extra_feat, combined
        else:
            return logits
